...
\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump(
 array(
  ...
  'frontendUser' => $frontendUser,
            ...
            /*
            if((int)$registerSuccessPID>0) {
              $uri = $uriBuilder->setTargetPageUid((int)$registerSuccessPID)->build();
            }
            else {
    ...
    {
        $landlordLoginPid = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_dlmobilebankid_mobilebankid.']['settings.']['tenantLoginPid'];
        $landlordGroupuid = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_dlmobilebankid_mobilebankid.']['settings.']['tenantGroupuid'];
        $feUserStoragePID = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_felogin_pi1.']['storagePid'];
